...
Las restricciones del escenario funcional dejaron de producir solicitudes de mudanza después del mes 14. Ese caso sirve para comprobar las reglas, pero ofrece poco trabajo para estudiar paralelismo. Por ese motivo se creó la instancia \texttt{hpc.conf}, de $4096\times2560$ celdas y 240 iteraciones. Mantiene la distribución social, las preferencias y la regla económica, pero usa 20\% de viviendas vacías, permanencia cero, financiación de 60\%, plazo de 360 meses y un límite de cuota de 40\% del ingreso. Estas decisiones permiten una evolución más larga. La grilla es una carga sintética y no una estimación de la cantidad de viviendas de Montevideo.
...
El script \texttt{scripts/ejecutar\_experimentos.py} crea un directorio por ejecución, inicia las versiones secuencial e híbrida, compara los hashes y produce \texttt{resumen.csv}. Fija \texttt{OMP\_PLACES=cores}, \texttt{OMP\_PROC\_BIND=close} y la cantidad de hilos de cada configuración. Los resultados crudos no se versionan; el CSV usado por el informe se copia a \texttt{informe/datos} junto con la configuración necesaria para repetirlo.
